---
title: "Exercise 07 - An introduction to structural equation modeling (SEM)"
...
Also, exogenous-only variables are set to correlate by default in lavaan, so these paths do not need to be specified if you agree with them. If you want to fix these correlations between exogenous variables to zero instead, here is a tutorial on how to do that: https://lavaan.ugent.be/tutorial/syntax2.html


```{r example model specification}
...
```{r example model visualization 2}
...
```{r example not identified model}
...
- In the **Regressions** section: the standardized estimates show the standardized beta coefficients. These are interpreted just like standardized beta coefficients in linear regression. Their values should be between -1 to +1.
